...
describe("Custom Tests", function () {
  ...
  it("[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances", async () => {
    ...
    const alicesKeys = new Keypair();

    // deposit 1 ether by alice into the pool
    const depositAmount = utils.parseEther("0.1");
    const utxo = new Utxo({ amount: depositAmount, keypair: alicesKeys });
    ...
    await omniBridge.execute([
      ...
      { who: tornadoPool.address, callData: onTokenBridgedTrans.data },
    ]);
  ...
  it("[assignment] iii. see assignment doc for details", async () => {
    ...
    // private and public ke generation for alice and bob
    ...
    const bobSendAmount = utils.parseEther("0.08");
    ...
    await transaction({
      ...
      outputs: [aliceChangeUtxo, bobSendUtxo],
      isL1Withdrawal: false,
    });

    // Detection of incoming funds by bob
    ...
    // Withdrawal by bob from the shielded pool
    ...
    // withdrawal by alice from the shielded pool
    const aliceWithdrawAmount = aliceChangeUtxo.amount;
    const aliceEthAddress = "0x5534c16131B5DFa88014149e1F9E7A777A630558";
